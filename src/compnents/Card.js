...
export default function Card({ filterr, obj, user }) {
  const priority_map = [
    ...
    <i
      style={{ color: "#fc7840" }}
      className="bx bxs-message-alt-error stylep"
    ></i>,
  ];
  const status_map = {
    Backlog: (
  ...
  return (
    <div className="card-container box-shadow">
      <div className="card-head">
        <span>{obj.id}</span>
        <ProfileImage
          name={user[obj.userId].name}
          available={user[obj.userId].available}
        />
      </div>
      <h3>
        {" "}
        {filterr?.grouping !== "status" && status_map[obj.status]} {obj.title}
      </h3>
      <div style={{ display: "flex", alignItems: "center", borderWidth: 1 }}>
        {filterr?.grouping !== "priority" && priority_map[obj.priority]}
        {obj.tag.map((ele, i) => (
          <p key={i}>
            <i
              style={{ color: "#bec2c8", marginRight: "4px" }}
